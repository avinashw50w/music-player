    ...
    getPresets() {
        return {
            ...
            dots: (ctx, canvas, data, len, hue, instance) => {
                ...
                // MIRRORING FIX: Iterate half, draw mirrored to ensure continuity at 3 o'clock
                const particles = 60; // Represents half circle density (originally 120 for full)
                for (let i = 0; i <= particles; i++) {
                    const dataIndex = Math.floor(i * (len / 3) / particles);
                    const value = timeData[dataIndex];
                    
                    const angle = (i / particles) * Math.PI; // 0 to PI
                    ...
                    // Mirror, but don't double draw start(0) and end(PI)
                    if (i > 0 && i < particles) {
                        drawDot(-angle);
                    }
                }
            ...
            circular_beat: (ctx, canvas, data, len, hue, instance) => {
                ...
                // MIRRORING FIX: Use half the buffer and mirror
                const points = Math.floor(len / 2);
                
                for (let i = 0; i <= points; i++) {
                    let v = timeData[i];
                    
                    // Map 0..points to 0..PI
